...
func (a *Aggregator) GetAllCurrencies(ctx context.Context) (popular []models.Currency,
	...
	popularCurrenciesLookup := maps.New[models.NetworkPair, models.Currency]()
	restCurrenciesLookup := maps.New[models.NetworkPair, models.Currency]()
		...
		go func(ex Exchange) {
			...
			for _, curr := range currencies {
				if _, ok := popularSymbolLookup[curr.GetLowerSymbol()]; ok {
					popularCurrenciesLookup.Add(curr.GetFirstNetwork(), curr)
					continue
				}
				restCurrenciesLookup.Add(curr.GetFirstNetwork(), curr)
			}
			mapMu.Unlock()
		}(exchange)
	}
...
func (a *Aggregator) sortCurrenciesAndSetToCache(
	lookup maps.Map[models.NetworkPair, models.Currency], cacheKey string) {
